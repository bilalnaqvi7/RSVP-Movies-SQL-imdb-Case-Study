...
SELECT 
    COUNT(*) AS No_of_row_director_mapping
FROM
    director_mapping;
-- No_of_row_director_mapping = 3876

SELECT 
    COUNT(*) AS No_of_row_genre
FROM
    genre;
-- No_of_row_genre = 14662

SELECT 
    COUNT(*) AS No_of_row_movie
FROM
    movie;
-- No_of_row_movie = 7997

SELECT 
    COUNT(*) AS No_of_row_names
FROM
    names;
-- No_of_row_names = 25735

SELECT 
    COUNT(*) AS No_of_row_ratings
FROM
    ratings;
-- No_of_row_ratings = 7997

SELECT 
    COUNT(*) AS No_of_row_role_mapping
FROM
    role_mapping;
...
-- By observation, German movies received the highest number of votes when queried against language and country colum
...
final_tab AS (
    SELECT
        ...
        ROW_NUMBER() OVER (PARTITION BY m.year ORDER BY worldwide_gross_income DESC) AS movie_rank
    FROM
        movie m
    INNER JOIN
        genre g ON g.movie_id = m.id
